...
enum TicketNewError {
    TitleError{ error_message: String},
    DescriptionError,
}
...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description.clone(), status.clone()){
        Ok(ticket) => ticket,
        Err(TicketNewError::TitleError{error_message}) => panic!("{error_message}"),
...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TicketNewError::TitleError{error_message: "Title cannot be empty".to_string()});
        }
        if title.len() > 50 {
            return Err(TicketNewError::TitleError { error_message: "Title cannot be longer than 50 bytes".to_string()});
        }
